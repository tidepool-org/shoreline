...
const (
	...
	testFailsFlag     = true
	testUserID        = "123-99-100"
	shorelineServerID = "shoreline"
)
...
func initTestBaseAPI(config ApiConfig, store Storage, metrics highwater.Client) *Api {
	return initTestAPIWithPerms(config, store, metrics, nil)
}
...
func createUserToken(t *testing.T) *SessionToken {
	return T_CreateSessionToken(t, testUserID, false, testTokenDuration)
}

func createServerToken(t *testing.T) *SessionToken {
	return T_CreateSessionToken(t, shorelineServerID, true, testTokenDuration)
}
...
func T_CreateSessionToken(t *testing.T, userId string, isServer bool, duration int64) *SessionToken {
	config := TokenConfig{DurationSecs: testAPIConfig.TokenDurationSecs, Secret: testAPIConfig.Secret}
	sessionToken, err := CreateSessionToken(&TokenData{UserId: userId, IsServer: isServer, DurationSecs: duration}, config)
	if err != nil {
...
func Test_GetUsers_Error_NotserverToken(t *testing.T) {
	sessionToken := createUserToken(t)
...
func Test_CreateUser_Success(t *testing.T) {
	...
	if response.Header().Get(tokens.TidepoolSessionTokenName) == "" {
		t.Fatalf("Missing expected %s header", tokens.TidepoolSessionTokenName)
	}
}
...
func Test_CreateCustodialUser_Error_MissingSessionToken(t *testing.T) {
	...
	response := T_PerformRequestBody(t, "POST", "/user/"+testUserID+"/user", body)
	T_ExpectErrorResponse(t, response, 401, "Not authorized for requested operation")
}
...
func Test_CreateCustodialUser_Error_MissingDetails(t *testing.T) {
	...
	headers := http.Header{}
	headers.Add(tokens.TidepoolSessionTokenName, sessionToken.ID)
	response := T_PerformRequestBodyHeaders(t, "POST", "/user/"+testUserID+"/user", body, headers)
	T_ExpectErrorResponse(t, response, 400, "Invalid user details were given")
}
...
func Test_UpdateUser_Error_UnauthorizedRoles_User(t *testing.T) {
	...
	responsableStore.FindTokenByIDResponses = []FindTokenByIDResponse{{token, nil}}
	responsableStore.FindUserResponses = []FindUserResponse{{&User{Id: testUserID}, nil}}
	defer T_ExpectResponsablesEmpty(t)

	body := `{"updates": {"username": "a@z.co", "emails": ["a@z.co"], "roles": ["clinic"]}}`
	headers := http.Header{}
	headers.Add(tokens.TidepoolSessionTokenName, token.ID)
	response := T_PerformRequestBodyHeaders(t, "PUT", "/user/"+testUserID, body, headers)
	T_ExpectErrorResponse(t, response, 401, "Not authorized for requested operation")
}
...
func TestDeleteUser_StatusForbidden_WhenNoPw(t *testing.T) {
	...
	request.Header.Set(tokens.TidepoolSessionTokenName, token.ID)
	response := httptest.NewRecorder()
...
func TestDeleteUser_Failure(t *testing.T) {
	...
	req.Header.Set(tokens.TidepoolSessionTokenName, token.ID)
...
func TestDeleteUser_StatusAccepted(t *testing.T) {
	...
	api.DeleteUser(response, request, map[string]string{"userid": testUserID})
...
func TestServerLogin_StatusBadRequest_WhenNoSecret(t *testing.T) {
	request, _ := http.NewRequest("POST", "/", nil)
	request.Header.Set(TP_SERVER_NAME, shorelineServerID)
	response := httptest.NewRecorder()
	api := initTestBaseAPI(testAPIConfig, mockStore, mockMetrics)
...
func TestServerLogin_Failure(t *testing.T) {
	...
	req.Header.Set(TP_SERVER_NAME, shorelineServerID)
...
func TestRefreshSession_StatusUnauthorized_WithWrongToken(t *testing.T) {
	...
	request, _ := http.NewRequest("GET", "/", nil)
	request.Header.Set(tokens.TidepoolSessionTokenName, "not this token")
	response := httptest.NewRecorder()
...
func TestRefreshSession_StatusOK(t *testing.T) {
	...
	refreshRequest.Header.Set(tokens.TidepoolSessionTokenName, token.ID)
	response := httptest.NewRecorder()
	...
	tokenString := response.Header().Get(tokens.TidepoolSessionTokenName)
	...
	if tokenData.UserId != testUserID {
		t.Fatalf("should have had a user id of `%v` but was %v", testUserID, tokenData.UserId)
	}
}
...
func TestHasserverToken_True(t *testing.T) {
	...
	svrLoginRequest.Header.Set(TP_SERVER_NAME, shorelineServerID)
	...
	if response.Code != http.StatusOK {
		t.Fatalf("Code expected:%v got:%v ", http.StatusOK, response.Code)
	}
}

func TestServerCheckSessionToken_StatusOK(t *testing.T) {

	serverToken := createServerToken(t)
	token := createUserToken(t)
	responsableStore.FindTokenByIDResponses = []FindTokenByIDResponse{{serverToken, nil}, {token, nil}}
	defer T_ExpectResponsablesEmpty(t)

	api := initTestBaseAPI(testAPIConfig, mockStore, mockMetrics)

	checkTokenRequest, _ := http.NewRequest("GET", "/", nil)
	checkTokenRequest.Header.Set(tokens.TidepoolSessionTokenName, serverToken.ID)
	checkTokenResponse := httptest.NewRecorder()

	api.ServerCheckSessionToken(checkTokenResponse, checkTokenRequest, map[string]string{"token": token.ID})

	if checkTokenResponse.Code != http.StatusOK {
		t.Fatalf("Response Code expected:%v found:%v", http.StatusOK, checkTokenResponse.Code)
	}
	...
	if tokenData.IsServer == true {
		t.Fatalf("should have been a user token but was %v", tokenData.IsServer)
	}

	if tokenData.UserId != testUserID {
		t.Fatalf("should have had a id of `%s` but was %v", testUserID, tokenData.UserId)
	}

}

func TestServerCheckSessionToken_StatusUnauthorized_WhenNoSvrToken(t *testing.T) {
	...
	api.ServerCheckSessionToken(response, request, testNoParams)
...
func Test_AuthenticateSessionToken_Missing(t *testing.T) {
	...
	response := httptest.NewRecorder()
	api := initTestAPIWithPerms(testAPIConfig, responsableStore, mockMetrics, responsableGatekeeper)
	tokenData := api.jwtValidator.CheckToken(response, makeSessionRequest(""), "")
	if response.Code != http.StatusUnauthorized {
		t.Logf("Response Code expected:%d given:%d", http.StatusUnauthorized, response.Code)
		t.Fail()
	}
	if tokenData != nil {
		t.Log("Token should be nil")
		t.Fail()
	}
}
...
func Test_AuthenticateSessionToken_Invalid(t *testing.T) {
	...
	tokenData := api.jwtValidator.CheckToken(response, makeSessionRequest("xyz"), "")
	if response.Code != http.StatusUnauthorized {
		t.Logf("Response Code expected:%d given:%d", http.StatusUnauthorized, response.Code)
		t.Fail()
	}
...
func Test_AuthenticateSessionToken_Expired(t *testing.T) {
	response := httptest.NewRecorder()
	sessionToken := T_CreateSessionToken(t, testUserID, false, -3600)
	responsableStore.FindTokenByIDResponses = []FindTokenByIDResponse{{nil, nil}}
	defer T_ExpectResponsablesEmpty(t)

	api := initTestAPIWithPerms(testAPIConfig, responsableStore, mockMetrics, responsableGatekeeper)
	tokenData := api.jwtValidator.CheckToken(response, makeSessionRequest(sessionToken.ID), "")
	if response.Code != http.StatusUnauthorized {
		t.Logf("Response Code expected:%d given:%d", http.StatusUnauthorized, response.Code)
		t.Fail()
	}
...
func Test_AuthenticateSessionToken_Success_User(t *testing.T) {
	...
	if response.Code == http.StatusUnauthorized {
		t.Logf("Response Code given:%d", response.Code)
		t.Fail()
	}

	if tokenData == nil {
		t.Log("Missing expected token data")
		t.Fail()
	}
	if tokenData.UserId != testUserID {
		t.Logf("Token userID: expoected:%s given:%s", testUserID, tokenData.UserId)
		t.Fail()
	}
...
func Test_AuthenticateSessionToken_Success_Server(t *testing.T) {
	...
	if tokenData.UserId != shorelineServerID {
		t.Logf("Unexpected token user id: %s", tokenData.UserId)
		t.Fail()
	}
	if !tokenData.IsServer {
		t.Log("Unexpected non-server token")
		t.Fail()
	}
	if tokenData.DurationSecs != testTokenDuration {
		t.Logf("Unexpected token duration: %d", tokenData.DurationSecs)
		t.Fail()
	}
}
...
func Test_TokenUserHasRequestedPermissions_Server(t *testing.T) {
	tokenData := &TokenData{UserId: testUserID, IsServer: true, DurationSecs: testTokenDuration}
...
func Test_TokenUserHasRequestedPermissions_Owner(t *testing.T) {
	tokenData := &TokenData{UserId: testUserID, IsServer: false, DurationSecs: testTokenDuration}
	requestedPermissions := clients.Permissions{"a": clients.Allowed, "b": clients.Allowed}
	api := initTestAPIWithPerms(testAPIConfig, responsableStore, mockMetrics, responsableGatekeeper)
	permissions, err := api.tokenUserHasRequestedPermissions(tokenData, testUserID, requestedPermissions)
	if err != nil {
		t.Fatalf("Unexpected error: %#v", err)
	}
	if !reflect.DeepEqual(permissions, requestedPermissions) {
